...
| `POST ?id=X&action=split` (+`preview=1`) `{"from_date", …Seriendefinition}` | „Dieser und alle folgenden" **mit** Regeländerung: alte Serie endet am Vortag von `from_date` (wie DELETE unten), neue Serie ab `from_date` (wie POST). Die `exdates` der alten Serie ab `from_date` gehen in die neue über — einzeln gelöschte Termine kommen nicht zurück (die Vorschau zeigt sie abgewählt). Eine Transaktion. Vorschau zusätzlich mit `removes` (werden ersetzt) und `keeps` (Termine mit Daten, bleiben abgelöst stehen). Antwort wie POST, ergänzt um `removed`, `detached` und `series_deleted` |
